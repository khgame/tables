...
function setActiveEventType(type, silent = false) {
    if (activeEventType === type)
        return;
...
function renderGroupSuggestions(role, currentGroup) {
    if (!groupSuggestions)
        return;
    if (!role) {
...
function getSquarePoints(center, radius) {
    const points = [];
...
function getFloodFillPoints(start, target, replacement) {
    ...
    while (queue.length) {
        ...
        if (visited.has(id))
            continue;
        visited.add(id);
        if (cell.row < 0 || cell.row >= state.board.rows || cell.col < 0 || cell.col >= state.board.cols)
            continue;
