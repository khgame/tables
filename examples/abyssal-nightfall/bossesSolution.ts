...
import { IBosses, BossesTID, toBossesTID, BossesRepo } from "./bosses";
...
export const bossesRaw = raw;
export const bossesTids: BossesTID[] = raw.tids.map(toBossesTID);
export const bossesRecords: Record<BossesTID, IBosses> = Object.fromEntries(
  Object.entries(raw.result).map(([tid, value]) => [toBossesTID(tid), value as IBosses])
);
export const bosses = bossesRecords;
export const bossesRepo = BossesRepo.fromRaw(raw);
