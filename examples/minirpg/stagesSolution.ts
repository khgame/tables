...
import { IStages, StagesTID, toStagesTID, StagesRepo } from "./stages";
...
export const stagesRaw = raw;
export const stagesTids: StagesTID[] = raw.tids.map(toStagesTID);
export const stagesRecords: Record<StagesTID, IStages> = Object.fromEntries(
  Object.entries(raw.result).map(([tid, value]) => [toStagesTID(tid), value as IStages])
);
export const stages = stagesRecords;
export const stagesRepo = StagesRepo.fromRaw(raw);
