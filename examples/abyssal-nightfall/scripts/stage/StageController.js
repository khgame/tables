import { resolveEnemyTemplate } from '../core/library.js';
import { pushLog } from '../battle/logger.js';
import { DEFAULT_ENEMY_RADIUS } from '../battle/constants.js';
import { EnemyUnit } from '../battle/entities.js';
...
export class StageController {
    constructor(state, dom, assets) {
        this.state = state;
        this.dom = dom;
        this.assets = assets;
        this.missingEnemySpriteLogged = new Set();
    }
    prepare(library) {
    ...
    update() {
        while (this.state.nextWaveIndex < this.state.waves.length &&
        ...
        for (let i = 0; i < wave.count; i++) {
            ...
            if (!sprite && spritePath && !this.missingEnemySpriteLogged.has(spritePath)) {
                ...
                void this.assets.loadImage(spritePath).then(img => {
                    if (img)
                        this.missingEnemySpriteLogged.delete(spritePath);
                });
            }
            const enemy = new EnemyUnit(template, { x, y }, { radius: enemyRadius, sprite, spritePath, spriteScale: template.spriteScale || 1 });
