import { normalizeSlug, normalizeIdentifier } from '../core/utils.js';
import { pushLog } from '../battle/logger.js';
const xpForLevel = (level) => Math.round(60 * Math.pow(level, 1.35));
export class ProgressionManager {
    constructor(state, dom, library, assets) {
        this.state = state;
        this.dom = dom;
    ...
    setStatsChangedHandler(handler) {
    ...
    processQueue() {
        if (this.state.mode !== 'playing')
            return;
        if (!this.state.upgradeQueue.length)
            return;
    ...
    openUpgradePanel(job) {
        ...
        options.forEach(option => {
            ...
            if (isSkill) {
                ...
                if (branchName)
                    card.classList.add(`branch-${branchName}`);
            }
            else {
                card.classList.add('synergy');
                const tier = data.tier;
                if (tier)
                    card.classList.add(tier.toLowerCase());
            }
            const iconPath = data.icon || data.icon || null;
            const resolvedIcon = this.assets.resolvePath(iconPath);
            const meta = isSkill
                ? `分支 ${data.branchName || data.branch || '未分类'} · 阶段 ${data.tier ?? '—'}`
                : `品质 ${data.tier ?? '—'}`;
            const effects = this.formatEffects(data.effects || data.effects || '');
            card.innerHTML = `
    ...
    buildUpgradeOptions(reason) {
        ...
        if (skillCandidates.length >= 3) {
            options.push(...skillCandidates.slice(0, 3).map(node => ({ type: 'skill', data: node, description: node.tooltip || '技能强化' })));
        }
        else if (skillCandidates.length > 0) {
            options.push(...skillCandidates.map(node => ({ type: 'skill', data: node, description: node.tooltip || '技能强化' })));
            if (options.length < 3 && synergyCandidates.length) {
                options.push(...synergyCandidates
                    .slice(0, 3 - options.length)
                    .map(card => ({ type: 'synergy', data: card, description: card.trigger || '协同效果常驻生效。' })));
            }
        }
    ...
    getAvailableSkills() {
        ...
        return skillTree.filter(node => {
            if (this.state.unlockedSkills.has(node.tid))
                return false;
            if (node.parent) {
                const parentId = node.parent.split(':')[1] || node.parent;
                if (!this.state.unlockedSkills.has(parentId))
                    return false;
            }
    ...
    getAvailableSynergies() {
        ...
        return synergyCards.filter(card => {
            if (this.state.unlockedSynergies.has(card.tid))
                return false;
    ...
    requirementsMet(requirements) {
        ...
        return requirements.split('|').every(token => {
            ...
            if (!value)
                return true;
            if (kind === 'level')
                return this.state.level >= Number(value);
            if (kind === 'skill')
    ...
    applyUpgradeOption(option) {
        if (option.type === 'skill') {
            this.unlockSkill(option.data);
        }
        else {
            this.unlockSynergy(option.data);
        }
        this.notifyStatsChanged();
    }
    unlockSkill(node) {
