...
export const goSerializer: Serializer = {
  ...
  file: (data, fileName, _imports, context) => {
    ...
    const importSection = renderImports({ tidAware, usesBigInt })
    const bigIntHelpers = usesBigInt ? renderBigIntDeclarations() + '\n' : ''
    const goType = renderRootType(typeName, model, tidAware)

    return `// Code generated by tables. DO NOT EDIT.\n\npackage tables\n\n${importSection}${bigIntHelpers}${tidDefinitions}${goType}\n`
  }
}
...
function renderImports(input: { tidAware: boolean; usesBigInt: boolean }): string {
  const imports = new Set<string>()
  if (input.tidAware) {
    // No additional imports required for TID helpers currently.
  }
  if (input.usesBigInt) {
    imports.add('"fmt"')
  ...
  const ordered = Array.from(imports).sort()
  const body = ordered.map(entry => `  ${entry}`).join('\n')
  return `import (\n${body}\n)\n\n`
}

function renderBigIntDeclarations(): string {
  return `type BigIntStr string\n\nfunc (b BigIntStr) BigInt() (*big.Int, error) {\n  value, ok := new(big.Int).SetString(string(b), 10)\n  if !ok {\n    return nil, fmt.Errorf("invalid BigIntStr %q", string(b))\n  }\n  return value, nil\n}\n\nfunc (b BigIntStr) MustBigInt() *big.Int {\n  value, err := b.BigInt()\n  if err != nil {\n    panic(err)\n  }\n  return value\n}\n\nfunc (b BigIntStr) Int64() (int64, error) {\n  value, err := b.BigInt()\n  if err != nil {\n    return 0, err\n  }\n  if !value.IsInt64() {\n    return 0, fmt.Errorf("BigIntStr %q exceeds int64 range", string(b))\n  }\n  return value.Int64(), nil\n}\n`
}
