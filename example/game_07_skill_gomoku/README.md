...
- **卡牌速度（新版定义）**:
  - `Instant - 瞬发`：发动后“无需选择目标”（不需要点格子、不需要点快照、不需要点对方卡牌）。这类技能进入反击窗口后，玩家有 7 秒可点击“取消”，也可立即点“发动”立刻生效；超过 7 秒将自动生效（仍允许对手在反击窗口内反击）。
  - `Normal - 非瞬发`：发动需要选择目标（格子/快照/卡牌）。选择完成后立即进入反击窗口，玩家侧不显示倒计时（可随时点击“取消”撤回），到达反击窗口超时时自动结算。
...
| 卡牌 | 张数 | 描述 | 备注 | 卡牌宣言 |
| --- | --- | --- | --- | --- |
| 飞沙走石 (Sandstorm) | 4 | 指定敌方棋子移出棋盘，进入"什刹海"；封禁该落点一回合，同时自己放弃本回合落子 | 非瞬发（需要选点）；可被“拾金不昧”“擒拿”应对。 | 你管这个叫五子棋？ |
| 静如止水 (Time Freeze) | 1 |冻结敌方，跳过对方下一轮落子或发动魔法卡 (反击卡不受影响)。 | 对应反制：水滴石穿。 | 嗯嗯，嗯 |
| 时光倒流 (Time Rewind) | 1 |悔棋：移除双方最近一轮的落子（各移除一手；若某方无落子则忽略），不还原已使用的卡牌/手牌/墓地/牌堆。 | 瞬发（无需选点）；多面控制。 | 这一招好 |
| 这是棒球 (Baseball Strike) | 1 | 与飞沙走石相同，合体技，免疫擒拿/喝止；拾金不昧仍可事后挽回棋子。 | 非瞬发（需要选点）；仅“拾金不昧”能事后挽回。 | 呀咧呀咧 |
